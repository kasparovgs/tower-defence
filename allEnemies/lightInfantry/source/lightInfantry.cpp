...
static bool isWalkable(int x, int y, Matrix matr){
    return matr[x][y].getFieldType() == fieldType::Plain && matr[x][y].getStructure() == nullptr;
}


void LightInfantry::findPath(){
...
static bool checkMissChance(std::size_t missChance_) {
    std::random_device rd;
    std::mt19937 gen(rd());
    std::uniform_int_distribution<std::size_t> dis(0, 99);

    std::size_t randomValue = dis(gen);

    return randomValue < missChance_;
}


void LightInfantry::takeDamage(std::size_t damage) {
    if (!checkMissChance(missChance_)){
        // setCurrentHP((damage > getCurrentHP()) ? 0 : getCurrentHP() - damage);
...
std::shared_ptr<EnemyStats> LightInfantry::getStats() const {
    return std::make_shared<LightInfantryStats>(getMaxHP(),
                                                getRegenerationRate(),
                                                getSpeed(),
                                                getMissChance());
}